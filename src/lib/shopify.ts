...
export async function publishToShopify(product: Product): Promise<any> {
    ...
    if (!response.ok) {
        ...
        if (response.status === 401) {
             errorMessage = `Authentication failed. The provided Admin API Access Token is invalid or does not have the required permissions (e.g., 'write_products'). Please verify your 'SHOPIFY_ADMIN_ACCESS_TOKEN' secret.`;
        }
