...
async function erpFetch(endpoint: string, init?: RequestInit) {
  try {
    ...
    if (!response.ok) {
        ...
        // Try to parse the specific ERPNext server message for better feedback
        if (errorBody._server_messages) {
            try {
                const serverMessage = JSON.parse(errorBody._server_messages[0]);
                errorMessage = serverMessage.message || JSON.stringify(serverMessage);
            } catch (e) {
                // Fallback if parsing fails
                errorMessage = errorBody._error_message || JSON.stringify(errorBody);
            }
        } else if (errorBody.error) {
           errorMessage = errorBody.error;
        } else if (errorBody.exc_type) {
           errorMessage = `${errorBody.exc_type}: ${errorBody._error_message || 'No details provided.'}`;
        }
...
export async function importProductsFromERPNext(
  ...
  try {
    ...
    const upsertedProducts = await Promise.all(allItems.map(async (item: any) => {
        ...
        const images: ProductImage[] = imageUrl ? [{ url: imageUrl, isMain: true }] : [];
  ...
  try {
    ...
    const updatedProductsPromises = currentProducts.map(async (p: Product) => {
      ...
      try {
        ...
        const newImages: ProductImage[] = newImage ? [{ url: newImage, isMain: true }] : [];

        // Simple comparison for image update check
        if (newImage && (!p.images[0] || p.images[0].url !== newImage)) {
            newProductData.images = newImages;
            hasUpdate = true;
        }
